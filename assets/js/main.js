document.addEventListener("DOMContentLoaded", () => {
    ...
    Promise.all(jsonFiles.map(file => fetch(file).then(response => response.json())))
      .then(dataArrays => {
        ...
        displayDishes(allDishes.slice(0, 4), "featuredDishes"); // Hiển thị 4 món đầu tiên
  ...
  function filterByCategory(category) {
    ...
    Promise.all(jsonFiles.map(file => fetch(file).then(response => response.json())))
      ...
      .catch(error => console.error("Lỗi khi lọc món:", error));
  }
  ...
  function searchDishes() {
    ...
    Promise.all(jsonFiles.map(file => fetch(file).then(response => response.json())))
      ...
      .catch(error => console.error("Lỗi khi tìm món:", error));
  }
  
    // Hàm hiển thị chi tiết món ăn
  function showDishDetail(id) {
      window.location.href = `/pages/Congthuc/chitiet.html?id=${id}`;
  }
